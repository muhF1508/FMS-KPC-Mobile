  ...
  ({globalData, updateGlobalData, setActiveTab, apiService}, ref) => {
    ...
    useImperativeHandle(ref, () => ({
      ...
      getTimersState: () => ({
        ...
        currentTruckNumber: currentTruckNumber,
        loadCount: loadCount,
        workData: globalData.workData,
      }),
    }));

    // Restore state when component mounts or when coming back to this tab
    useEffect(() => {
      const persistentTimers = globalData.persistentTimers?.work;
      if (persistentTimers && persistentTimers.stepTimers) {
        console.log('🔄 Restoring work timers state:', persistentTimers);

        setStepTimers(persistentTimers.stepTimers);
        secondsRef.current = persistentTimers.seconds || secondsRef.current;
        setCurrentTruckNumber(persistentTimers.currentTruckNumber || 1);
        setLoadCount(persistentTimers.loadCount || 0);

        // Restore work data if exists
        if (persistentTimers.workData) {
          updateGlobalData({
            workData: persistentTimers.workData,
          });
        }

        // Restore active activity if exists
        if (persistentTimers.activeStep && persistentTimers.currentActivity) {
          setActiveStep(persistentTimers.activeStep);
          setCurrentActivity(persistentTimers.currentActivity);
          ...
          // Resume timer for active step
          resumeStepTimer(persistentTimers.activeStep);
        }
      }
    }, []);
    ...
    const saveActivityToBackend = async (activityName, startTime, endTime) => {
      ...
      try {
        ...
        console.error(
          `💥 Error saving ${activityName} activity:`,
          error.message,
        );
        return false;
      } finally {
    ...
    const resumeStepTimer = stepName => {
      if (intervalRef.current) {
        clearInterval(intervalRef.current);
      }
      ...
      console.log(`🔄 Resumed work step timer: ${stepName}`);
    };

    const startStepTimer = async stepName => {
      // Stop previous activity if any (silently)
      if (currentActivity && activityStartTime) {
    ...
    const stopCurrentActivity = async (showAlert = true) => {
      ...
      if (showAlert) {
        if (saved) {
          Alert.alert(
            'Aktivitas Selesai',
            `${currentActivityName} telah dihentikan dan disimpan`,
          );
        } else {
          Alert.alert(
            'Aktivitas Selesai (Offline)',
            `${currentActivityName} telah dihentikan\n\n⚠️ Data tersimpan lokal, akan disinkronkan saat online`,
          );
        }
      } else {
        console.log(
          `✅ ${currentActivityName} completed silently - ${
            saved ? 'Saved' : 'Offline'
          }`,
        );
      }
      ...
      try {
        if (globalData.documentNumber) {
          ...
          await saveActivityToBackend(
            `TRUCK_COMPLETED_${currentTruckNumber}`,
